...
from ..utils.utils import get_access_token, get_services, post_query_service, query_service, parse_results
...
async def post_beacon_query(request):
    ...
    for service in services:
        # Generate task queue
        task = asyncio.ensure_future(post_query_service(service, params, access_token, request.headers.get("json")))
        tasks.append(task)
